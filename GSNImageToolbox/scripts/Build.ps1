...
function GetVersion()
{
  ...
  foreach ($line in [System.IO.File]::ReadLines("$PSScriptRoot/../3pp/VisualMagick/installer/inc/version.isx"))
  {
...
{
  Set-Location 3pp/VisualMagick/configure
  & 'C:\Program Files (x86)\Microsoft Visual Studio 14.0\Common7\IDE\devenv' /upgrade configure.vcxproj
  & 'C:\Program Files (x86)\MSBuild\14.0\Bin\msbuild' configure.sln /m:4 /t:Build ("/p:Configuration=Release,Platform=Win32")
...
function BuildConfiguration($config)
{
  ...
  Set-Location ..
  & 'C:\Program Files (x86)\MSBuild\14.0\Bin\msbuild' $config.solution /m:4 /t:Build ("/p:Configuration=Release,Platform=$platformName")
